...
print_dist_spec_indented <- function(x, indent, ...) {
  ...
  for (i in seq_len(ndist(x))) {
    if (get_distribution(x, i) == "nonparametric") {
      ...
      if (!is.null(attr(single_dist, "cdf_max"))) {
        constrain_str["cdf_max"] <-
          paste("cdf_max:", attr(single_dist, "cdf_max"))
      }
...
#' and no `cdf_max` of its own raises an error; bound it first (e.g. with
#' [bound_dist()]).
  ...
  pmf_data <- lapply(seq_len(ndist(x)), function(i) {
    if (get_distribution(x, i) == "nonparametric") {
      ...
      pmf_dt <- lapply(dists, function(y) {
        ...
        if (is.infinite(max(y)) && cdf_max == 1) {
          cli_abort(
            c(
              "!" = "Can't plot a {.val {get_distribution(x, i)}} distribution
              with no finite range.",
              "i" = "Set a finite {.arg max} or a {.arg cdf_max} below 1
              (for example with {.fn bound_dist})."
            )
          )
        }
        pmf_args <- list(y, cdf_max = cdf_max, width = res)
...
is_constrained.dist_spec <- function(x, ...) {
  ...
  tol_constrained <- !is.null(cdf_max) && cdf_max < 1
  max_dist <- attr(x, "max")
