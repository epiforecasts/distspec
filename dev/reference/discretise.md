...
and the remaining PMF is renormalised to sum to one. A `cdf_max` below
`1` additionally trims the tail, keeping the distribution only up to its
`cdf_max` quantile.

### Fixed distributions

A [`Fixed()`](https://epiforecasts.io/distspec/dev/reference/Fixed.md)
