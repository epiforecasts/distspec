...
PMF). For a Dirichlet-backed uncertain delay, draws `samples` PMFs from
the alpha vector via
[`rdirichlet()`](https://epiforecasts.io/distspec/dev/reference/rdirichlet.md)
...
A `data.frame` with columns `sample`, `x`, `p`, `distribution`.
