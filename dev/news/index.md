...
- Added
  [`as_dist_spec()`](https://epiforecasts.io/distspec/dev/reference/as_dist_spec.md),
  a generic for converting other representations of a probability
  distribution to a `<dist_spec>`: a `<dist_spec>` is returned
  unchanged, a numeric PMF vector becomes a
  [`NonParametric()`](https://epiforecasts.io/distspec/dev/reference/NonParametric.md)
  and a single number a
  [`Fixed()`](https://epiforecasts.io/distspec/dev/reference/Fixed.md).
  Other packages can register methods for their own classes without a
  dependency in either direction.

### Deprecations

- The `cdf_cutoff` argument of
  ...
  (and of the distribution constructors, which pass it on) has been
  renamed to `cdf_max`: the CDF level up to which the distribution is
  kept, mirroring `max` on the probability axis (`max = 10` truncates at
  the value 10, `cdf_max = 0.999` at the 99.9% quantile). The semantics
  are unchanged. Supplying `cdf_cutoff` warns and interprets the value
  as `cdf_max`; a value in the historical EpiNow2 convention (the tail
  probability to drop) is caught by the existing guard against values
  below `0.5`, which points to the `1 - x` conversion. The internal
  attribute carrying the bound is renamed accordingly.
...
- [`Fixed()`](https://epiforecasts.io/distspec/dev/reference/Fixed.md)
  now forwards its `...` to
  [`bound_dist()`](https://epiforecasts.io/distspec/dev/reference/bound_dist.md)
  as documented; previously limits such as `Fixed(3, max = 5)` were
  silently dropped.
- A non-finite
  [`Fixed()`](https://epiforecasts.io/distspec/dev/reference/Fixed.md)
  value (e.g. `NA` or `Inf`) is now rejected with an informative error;
  previously it raised a bare condition error from the internal bound
  check.
- Bounding a fixed distribution below its value
  (e.g. `Fixed(6, max = 5)`) is now rejected, and discretising a
  partially truncated fixed distribution renormalises the PMF to sum to
  one; previously both silently produced an invalid PMF.
...
- The
  [`NonParametric()`](https://epiforecasts.io/distspec/dev/reference/NonParametric.md)
  help page now documents that a distribution constructed with a
  [`Dirichlet()`](https://epiforecasts.io/distspec/dev/reference/Dirichlet.md)
  prior has no concrete PMF until
  ...
  [`has_uncertainty()`](https://epiforecasts.io/distspec/dev/reference/has_uncertainty.md)
  now appears in the reference index to match.
...
- [`Exp()`](https://epiforecasts.io/distspec/dev/reference/Exponential.md)
...
- [`NonParametric()`](https://epiforecasts.io/distspec/dev/reference/NonParametric.md)
  ...
  and normalised, but now warn when they do not sum to one.
- A distribution parameter given as a certain distribution (standard
  ...
  [`sd()`](https://epiforecasts.io/distspec/dev/reference/sd.md)
  returned `NA` for an otherwise fully-fixed distribution
...
- [`mean()`](https://rdrr.io/r/base/mean.html) and
  [`sd()`](https://epiforecasts.io/distspec/dev/reference/sd.md) now
  emit an informative message when they return `NA` because a
  distribution has uncertain parameters, pointing to
  `mean(x, ignore_uncertainty = TRUE)` and
  [`fix_parameters()`](https://epiforecasts.io/distspec/dev/reference/fix_parameters.md).
- Improved the error messages from
  [`get_element()`](https://epiforecasts.io/distspec/dev/reference/get_element.md)
  and
  [`get_parameters()`](https://epiforecasts.io/distspec/dev/reference/get_parameters.md):
...
- Documentation improvements: the getting-started vignette now shows the
  ...
  [`discretise()`](https://epiforecasts.io/distspec/dev/reference/discretise.md),
  [`fix_parameters()`](https://epiforecasts.io/distspec/dev/reference/fix_parameters.md)
  and [`sd()`](https://epiforecasts.io/distspec/dev/reference/sd.md)
  help pages have clearer descriptions and runnable examples.
